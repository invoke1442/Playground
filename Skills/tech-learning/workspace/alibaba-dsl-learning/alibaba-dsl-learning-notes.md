...
**JS loadclass** 仍然通过 `source.customSourceFunc = loadclass(...)` 路径工作 [s22✅ g11✅]。
...
| `source.customSourceFunc` | 自定义函数 | `loadclass(...)` | g11 ✅ |

### 5.4 JS sink 可用子字段

JS sink 实际支持 **7 个子字段**（均仅限 Roster），block 语法只有 `methodArg`，其余全用 string 语法：
...
> `重要发现`: JS sink 的 string 字段不接受 block 语法 [x20-x25 全❌]，只有 `sink.methodArg` 用 block
> `容易踩坑`: `sink.methodArg` 不接受 string 语法 [y01❌]，必须用 block + `pattern`

### 5.4.1 JS sanitizer 可用子字段

JS sanitizer 支持 **6 个子字段**（均仅限 Roster），语法分布与其他角色不同：
...
2. **JS loadclass**: 在 JS Roster 中通过 `customSourceFunc`/`customSinkFunc` 加载自定义 JS 代码，可实现任意 AST 检查逻辑
...
| block 内字段 | precise + value | 额外支持 excludeTag, tag, flag, xpath, param(JSON), from, to, accurate 等 |
...
| `excludeTag` | 排除标签 (与 import exclude 联动) | `excludeTag = "networkType";` |
| `tag` | 标记标签 | `tag = "special_tag";` |
| `flag` | 标志标记 | `flag = "MCPTool";` |
| `xpath` | XPath 表达式 (AST 匹配) | `xpath = "//Expression[...]";` |
| `param` | JSON 参数位置描述 | `param = "[{'position':0,'tainted':true}]";` |
| `from` | 传播来源 (propagate 专用) | `from = "0";` / `from = "object";` |
| `to` | 传播目标 (propagate 专用) | `to = "return";` / `to = "1";` |
| `accurate` | 精确控制 (propagate 专用) | `accurate = true;` |
| `userDefineClass` | 自定义类 (general 专用) | `userDefineClass = loadclass("...");` |
| `includePlatforms` | 平台包含 (group 专用) | `includePlatforms = "*";` |
| `excludePlatforms` | 平台排除 (group 专用) | `excludePlatforms = "legacy";` |
...
> 注：原始实验共 47 次调用（含错误码测试和重复验证），上表按独立规则/roster 配置计数为 39 个有效实验点。
...
- **本文档中所有 [expN] / [fN] / [gN] 标记均指向 `workspace/experiments/results/` 下对应 JSON 文件**
